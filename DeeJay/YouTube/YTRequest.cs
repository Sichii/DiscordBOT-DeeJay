...
namespace DeeJay.YouTube
{
    ...
    internal class YTRequest
    {
        ...
        internal YTRequest(string query, bool isLive = false)
        {
            Query = query;
            IsLive = isLive;
        }
        ...
        internal async Task<YTResult> ExecuteAsync(bool cullExtended, CancellationToken token)
        {
            ...
            if ((!isURI || isYouTube) && File.Exists(CONSTANTS.COOKIES_PATH) &&
                (await File.ReadAllLinesAsync(CONSTANTS.COOKIES_PATH)).Length > 0)
                queryStr += $" --cookies {CONSTANTS.COOKIES_PATH}";

            if (IsLive && !isYouTube)
                return new YTResult(Query, string.Empty, string.Empty, TimeSpan.MinValue);

            var output = (await YTDL.RunAsync(queryStr, !IsLive, token)).ToArray();
            ...
            if (output.Length >= 3)
            {
                ...
                return new YTResult(Query, output[0], output[1], duration, IsLive);
            }
